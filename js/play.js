...
var playState = {
    ...
    onRightButtonDown: function (pointer, mouseEvent) {
        var selected = this.countSelected();
        var counter = 0;
        var x = pointer.event.x + game.camera.x
        var y = pointer.event.y + game.camera.y;
        var destination = this.getDestinationGrid(selected, x, y);

        this.americansGroup.forEach(function (soldier) {
            if (soldier.selected) {
                soldier.cancelMovement();
                // soldier.moveTo(x, y);
                soldier.moveTo(destination[counter].x, destination[counter].y);
                counter++;
            }
        });
    ...
    getDestinationGrid: function (selected, x, y) {
        ...
        if (selected == 1) {
            ...
            var gridX = x - (selected * 20);
            var gridY = y - (selected * 20);

            for (var n = 0; n < (selected * 2); n++) {
                for (var m = 0; m < (selected * 2); m++) {
                    destination = {x: gridX, y: gridY};
                    coordinates.push(destination);
                    gridY += 20;
                }
                gridX += 20;
